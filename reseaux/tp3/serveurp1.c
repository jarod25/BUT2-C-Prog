    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            if (get1 != (int) sizeof tailleIn) {
                fprintf(stderr, "Error read client to server\n");
                close(socket_client);
                return EXIT_FAILURE;
            }
            int tab[tailleIn];
            int get2 = read(socket_client, &tab, sizeof tab);
            if (get2 < 0) {
            ...
            if (put1 != (int) sizeof j) {
                fprintf(stderr, "Error write1 server to client\n");
                close(socket_serveur);
                return EXIT_FAILURE;
            }
            ...
            if (put2 != (int) sizeof tab) {
                fprintf(stderr, "Error write2 server to client\n");
                close(socket_serveur);
                return EXIT_FAILURE;
            }
        }
